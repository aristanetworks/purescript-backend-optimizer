...
const test3 = /* #__PURE__ */ (() => {
  const Monad0 = monadEffectState.Monad0();
  const Bind1 = Monad0.Bind1();
  const Apply0 = Bind1.Apply0();
  const map1 = Apply0.Functor0().map;
  const pure1 = Monad0.Applicative0().pure;
  return Bind1.bind(monadEffectState.liftEffect(Effect$dConsole.log("foo")))(() => Bind1.bind(monadEffectState.liftEffect(Effect$dRandom.randomInt(1)(10)))(i1 => Bind1.bind(x => () => Data$dTuple.$Tuple(
    x,
    x
  ))(one => Bind1.bind(map1(v => v + one | 0)(monadEffectState.liftEffect(Effect$dRandom.randomInt(1)(10))))(i2 => Bind1.bind(x => () => Data$dTuple.$Tuple(Data$dUnit.unit, i2))(() => Bind1.bind(Apply0.apply(map1(Data$dSemiring.intAdd)(monadEffectState.liftEffect(Effect$dRandom.randomInt(1)(10))))(monadEffectState.liftEffect(Effect$dRandom.randomInt(1)(10))))(i3 => Bind1.bind(x => {
    const s$p = x + i3 | 0;
    return () => Data$dTuple.$Tuple(s$p, s$p);
  })(result => pure1(Data$dShow.showIntImpl(i1 + result | 0)))))))))(2);
})();
...
export {monadEffectState, program1, program2, test1, test2, test3};
