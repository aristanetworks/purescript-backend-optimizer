...
test1 = runMaybeT do
  ...
  i2 <- map (_ + 4) $ MaybeT $ map (map (_ + 1)) $ liftEffect $ map Just $ Random.randomInt 1 10
  i3 <- (+) <$> (MaybeT $ liftEffect $ map Just $ Random.randomInt 1 10) <*> (MaybeT $ liftEffect $ map Just $ Random.randomInt 1 10)
  pure $ 1 + i1 + i2 + i3
