...
toTopLevelBackendBinding group env (Binding _ ident cfn) = do
  let evalEnv = Env { isAppHead: false, tryLevel: zero, currentModule: env.currentModule, evalExtern: makeExternEval env, locals: [], prevWasRewritten: true, punt: Set.empty, safeToRecurse: Set.empty, directives: env.directives, stopTrying: false }
